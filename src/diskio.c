...
/--------------------------------------------------------------------------/
/
/	Edited by John Greenwell for use with AVR, 05 March 2013.
/
/-------------------------------------------------------------------------*/
...
/* Define 1 if SPI initialized prior to SD use */
#define SPI_ALREADY	0

#define	INIT_PORT()	init_port()	/* Initialize MMC control port */
...
/*---------------------------------------------------------*/
/* User Provided ff_wtoupper() for FatFs module           */
/*---------------------------------------------------------*/
...
/*---------------------------------------------------------*/
/* User Provided ff_convert() for FatFs module           */
/*---------------------------------------------------------*/
...
void init_port() {
	...
	// Enable SPI, Master, speed fosc/16
	SPCR = (1<<SPE)|(1<<MSTR)|(1<<SPR0);
	SPSR = (1<<SPI2X);
#endif

	// Initiate with CS high, deselected
	SPI_CS_DDR |= (1<<SPI_CS);
...
)
{
	...
	do {
		...
		SPDR = d;		/* Send byte via SPI */
		while(!(SPSR & (1<<SPIF)));	/* Wait until SPI transmit complete */
	} while (--bc);
}
